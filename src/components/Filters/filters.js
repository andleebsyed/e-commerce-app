        ...
        <div className="filters-main-outer">
            ...
            <div className="filter-container" style={{ display: isVisible }}>
                <ul className="sort-radio-buttons">
                    <li>
                        <input type="checkbox" onClick={(event) => dispatch({ type: "REMOVE_OUT_OF_STOCK", payload: event.target.checked })} /><label>In Stock</label>
                    </li>

                    <li>

                        <input type="checkbox" onClick={(event) => dispatch({ type: "SHOW_FAST_DELIVERY_ONLY", payload: event.target.checked })} /><label>Fast Delivery</label>

                    </li>
                    <li>
                        <button className="clear-button" onClick={() => dispatch({ type: 'REMOVE_ALL_CONDITIONS' })} >Clear All</button>
